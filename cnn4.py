...
while True:
    ...
    if pred > 0.5:
        ...
        # Define variables for wave run-up and overtopping volumes
        sensor_height = 2  # Height of the sensor above sea level (in meters)
        sensor_width = 1  # Width of the sensor perpendicular to the wave direction (in meters)
        wave_runup_volume = 0  # Volume of water above the sensor due to wave run-up (in cubic meters)
        overtopping_volume = 0  # Volume of water above the sensor due to overtopping (in cubic meters)
        ...
        for c in cnts:
            ...
            # Calculate the wave run-up height and volume
            if prev_height is not None:
                height_diff = y - prev_height
                if height_diff > 0:
                    wave_height = height_diff
                    wave_runup_height = wave_height / 600  # Convert to meters
                    wave_runup_volume += sensor_width * wave_runup_height

            # Calculate the overtopping volume
            if y < sensor_height:
                overtopping_height = (sensor_height - y) / 600  # Convert to meters
                overtopping_volume = sensor_width * overtopping_height
                overtopping_volume = overtopping_volume if overtopping_volume <= sensor_width * wave_runup_height else sensor_width * wave_runup_height
                overtopping_volume -= wave_runup_volume
                if overtopping_volume > 0:
                    overtopping_volume = overtopping_volume * frame.shape[1] / 800 * 2  # Convert to cubic meters
                    overtopping_volume = overtopping_volume if overtopping_volume <= sensor_width * wave_runup_height else sensor_width * wave_runup_height

            # Draw a dot at the top of each contour
            cv2.circle(frame, (int(x + w / 2), y), 10, (0, 0, 0), -1)

            # Update the previous height
            prev_height = y

    # Display the wave run-up and overtopping volumes
    if wave_runup_volume > 0:
        cv2.putText(frame, "Wave Run-up Volume: {:.2f} m^3".format(wave_runup_volume), (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    if overtopping_volume > 0:
        cv2.putText(frame, "Overtopping Volume: {:.2f} m^3".format(overtopping_volume), (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
    ...
    if prev_frame is not None:
        ...
        if wave_height is not None:
            ...
            if wave_height > 50:
                cv2.putText(frame, "Small Wave Detected", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            else:
                cv2.putText(frame, "Large Wave Detected", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            ...
            # for i in range(num_dots+1):
                ...
                cv2.circle(frame, dot_pos, 5, (0, 255, 0), -1)
